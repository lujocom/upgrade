...
LOCAL_UPGRADE_FILE = "/Users/luohui/Desktop/local-upgrade/"
...
def ssh_put_all(ip, user, password, local_file_obj):
...
def shh_put_all_by_pysftp(ip, user, password, local_file_obj):
    print(local_file_obj)
...
def upload_upgrade_file():
    for ip in IP_LIST:
        ...
        for local_file in project_list:
            # ssh_put_all(ip, USER, PASSWORD, local_file)
            shh_put_all_by_pysftp(ip, USER, PASSWORD, local_file)
...
def cp_file_to_webapp():
    for ip in IP_LIST:
        ...
        for local_file in project_list:
            commend = 'cp -r ' + os.path.join(UPGRADE_PATH, local_file.name)
        print('***************cp upgrade file to normal***************\n')
    pass
